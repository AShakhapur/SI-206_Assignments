...
def create_request_url(title):
    ...
    param = {'t':title, 'type':'movie', 'r':'json', 'plot':'short', 'apikey':'67b52828'}
    request = requests.get(base_url, params = param)
    print(request.url)
    return request.url
...
def get_data_with_caching(title, CACHE_FNAME):
    ...
    """
    request_url = create_request_url(title)
    cache = read_cache(CACHE_FNAME)
    dic = {}
    if request_url in cache:
        print("Using cach for <"+title+">")
        return cache.get(request_url)
    elif request_url not in cache:
        print("Fetching data for <"+title+">")
        request = requests.get(request_url)
        request_dict = request.json()
        if request.status_code == 200 and request_dict['Response'] == 'True':
            cache[request_url] = request_dict
...
def movie_list(cost, CACHE_FNAME):
    ...
    movie_dict = {}
    for movie in cache: 
        results = movie.values()
        if int(results['BoxOffice'])>cost:
            movie_dict[results['Title']] = int(results['BoxOffice'])
    sorted_movies = dict(sorted(movie_dict.items(), key=lambda x: x[1]))
    return sorted_movies.keys()
...
class TestHomework7(unittest.TestCase):
    ...
    def test_movie_list(self):
        self.assertEqual(len(movie_list(15500000,self.CACHE_FNAME)), 22)
        self.assertEqual(len(movie_list(110000000,self.CACHE_FNAME)), 17)
        self.assertEqual(len(movie_list(40000000,self.CACHE_FNAME)), 21)
        self.assertEqual(movie_list(223225678,self.CACHE_FNAME)[0][0], 'Toy Story')
        self.assertEqual(movie_list(111540000,self.CACHE_FNAME)[0], ('The Little Mermaid', '111543479'))
...
def main():
    ...
    # print("EXTRA CREDIT!")
    # print("Movie list with BoxOffice cost more than $100,747,000")
    # print(movie_list(100747000,CACHE_FNAME))
    # print("________________________")
    
    # print("Movie list with BoxOffice cost more than $50,80,000")
    # print(movie_list(5080000,CACHE_FNAME))
    # print("________________________")
