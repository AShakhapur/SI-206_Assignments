...
class CsvReader(FileReader):
    ...
    def get_title_pages(self):
        ...
        for i in range(len(self.data_dict['Title'])):
            tuples_list.append(
                (self.data_dict['Title'][i], self.data_dict['Pages'][i])
                )
    ...
    def most_common_length(self):
        ...
        for i in self.data_dict['Pages']:
            ...
            length_counts[hundred_pages] = length_counts.get(hundred_pages, 0)

        top_length = sorted(length_counts.items())

        return str(top_length[0][0])+ "00"
